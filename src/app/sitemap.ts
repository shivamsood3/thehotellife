...
function contentDate(value: string | undefined, fallback: string): Date {
  ...
  const parsed = value ? Date.parse(/^[A-Za-z]+ \d{4}$/.test(value) ? `1 ${value}` : value) : NaN;
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const guideEntries: MetadataRoute.Sitemap = guides.map((g) => ({
    ...
    lastModified: contentDate(g.updated ?? g.date, "2026-08-01"),
    changeFrequency: "monthly",
    priority: 0.7,
  ...
  const articleEntries: MetadataRoute.Sitemap = articles.map((a) => ({
    ...
    lastModified: contentDate(a.updated ?? a.date, "2026-08-01"),
    changeFrequency: "monthly",
    priority: 0.7,
